...
import de.dertroglodyt.common.data.IDataAtom;
import de.dertroglodyt.common.data.IDataChangeListener;
...
import de.dertroglodyt.common.data.types.collection.DASet;
import de.dertroglodyt.common.data.types.collection.DAVector;
import de.dertroglodyt.common.data.types.compound.DAResult;
...
import de.dertroglodyt.iegcommon.core.DACharacter;
import de.dertroglodyt.iegcommon.core.DAClan;
import de.dertroglodyt.iegcommon.core.DALog;
import de.dertroglodyt.iegcommon.core.DAWorldNode;
import de.dertroglodyt.iegcommon.market.DAMarket;
import de.dertroglodyt.iegcommon.market.DAPatent;
import de.dertroglodyt.iegcommon.market.DAPerson;
import de.dertroglodyt.iegcommon.module.DAbmPropulsion.ThrusterType;
import de.dertroglodyt.iegcommon.moduleclass.DABasicModuleClass;
import de.dertroglodyt.iegcommon.moduleclass.DAbmPropulsionClass.Speed;
import de.dertroglodyt.iegcommon.moduleclass.DAmcShipClass;
import de.dertroglodyt.iegcommon.physic.DALocation;
import de.dertroglodyt.iegcommon.physic.DANewtonian;
import de.dertroglodyt.iegcommon.physic.DAOrbit;
import de.dertroglodyt.iegcommon.physic.DATransform;
import de.dertroglodyt.iegcommon.physic.DAVector3d;
...
public class DAShip extends DAModuleContainer implements IDataChangeListener {
...
//    }
    ...
    public static enum Parameters implements IParameterType {
        ...
        SHIP(de.dertroglodyt.iegcommon.module.DAShip.class),
        SHIPLIST(DAVector.class),  //<DAShip>
        NAME(DALine.class),
        DOCKINGSHIPID(DAUniqueID.class),
        THRUSTER_TYPE(DALine.class),
        THRUSTER_SPEED(DALine.class),
        PRESET_TYPE(DALine.class),
...
;
    ...
    public DAShip() {
        ...
        super.setName(new DALine("<not initialized>"));
        dockedTo = null;
...
//        scriptListener = null;
    ...
    public DAShip(DABasicModuleClass aWareClass, DATransform trans) {
        this(aWareClass, trans, new DALine("New " + aWareClass), new DAUniqueID());
    }

    public DAShip(DABasicModuleClass aWareClass, DATransform trans, DALine aName, DAUniqueID aOwnerID) {
...
//        scriptListener = scl;
        ...
        isPublicStation = false;
        fLog = new DALog();
        fLog.add(this.getItemID(), new DALine(aName + " gebaut."));
    }
...
//            worldNode.getAstroObjects().add(this);
        ...
        if (! resolving) {
            if (station != null) {
                fLog.add(station.getItemID(), new DALine("Angedockt an " + station.getName() + "."));
            } else {
                fLog.add(this.getItemID(), new DALine("Abgedockt."));
            }
        }
        notifyListener(this);
...
//            destID = null;
//        }
//        fLog.add(destID, new DALine("Ziel gesetzt: " + destination));
//    }
//
...
//    }
    ...
    public void setPublicStation(boolean isPublic) {
        ...
        if (isPublicStation) {
            fLog.add(this.getItemID(), new DALine("Schiff wurde als öffentliche Station deklariert."));
        } else {
            fLog.add(this.getItemID(), new DALine("Schiff ist keine öffentliche Station mehr."));
        }
    }
...
//            ship.setDockedTo(this);
        ...
        notifyListener(this);
        fLog.add(ship.getItemID(), new DALine("Neues Schiff angedockt: " + ship));
...
//            ship.setDockedTo(null);
        ...
        notifyListener(this);
        fLog.add(ship.getItemID(), new DALine("Schiff aus Hangar abgedockt: " + ship));
...
//                    s.resolveOther(this);
            ...
            for (DAbmRentableStorage rs : rentStorages) {
                ...
                if (s != null) {
//                    s.resolveOther(this);
                    fLog.add(leaserID, new DALine("Lagerraum gemietet von: " + c));
    ...
    public void setOrbit(DAOrbit orbit, DANewtonian central) {
        if (orbit != null) {
            ...
            worldNode.removeShip(this);
            fLog.add(central.getItemID(), new DALine("Orbit gesetzt um " + central + "."));
        } else {
            setState(State.IDLE);
            worldNode.addShip(this);
            fLog.add(central.getItemID(), new DALine("Orbit um " + central + " verlassen."));
        }
        notifyListener(this);
    }
...
//    }
    ...
    public synchronized void setOwner(DAClan c) {
        ...
        owner = c;
        ownerID = c.getUserID();
        fLog.add(c.getUserID(), new DALine("Neuer Besitzer: " + c + "."));
        if (owner != null) {
...
//                    , new DAPVMass(10000000), DAWareClass.Size.GIANT.getMaxVolume()*0.9), ownerID));
        ...
        market = new DAMarket(new DALine(toString() + " Market"));
        synchronized(market) {
            market.init(this);
        }
    }
...
//            captain.setScriptListener(scriptListener);
            val.setVessel(this);
            addToCrew(val);
            fLog.add(val.getID(), new DALine("Kapitän " + val + " an Bord."));
        } else {
            captainID = null;
            fLog.add(null, new DALine("Kapitän " + val + " von Bord."));
        }
        //notifyScriptListener();
        notifyListener(this);
    }
...
//            }
            ...
            //fLog.add(this.getItemID(), new DALine("Triebwerk " + type + " gesetzt auf " + speed + "."));
            return DAResult.createOK("ok.", "DAShip.setEngineSpeed");
        }
    }
...
//        }
        ...
        fLog.add(this.getItemID(), new DALine("Alle Triebwerke aus."));
    }
...
//        }
    ...
    public DAResult<DAVector<DAbmWaresContainer>> transferCargo(DAVector<DAbmWaresContainer> wares
        ...
        fLog.add(toShip.getItemID(), new DALine("Waren verladen auf Schiff " + toShip + "."));
        return rt;
    }

    public void setState(State s) {
        fState = s;
        fLog.add(this.getItemID(), new DALine("Status: " + s + "."));
    }
    ...
    public void setAimNone() {
        fAim = Aim.NONE;
        fLog.add(this.getItemID(), new DALine("Befehl: Keiner."));
    }

    public void setAimIdle() {
        if (fAim != Aim.TO_IDLE_2) {
            fAim = Aim.TO_IDLE;
            fLog.add(this.getItemID(), new DALine("Befehl: Alle Module auf Bereitschaft..."));
        }
    }

    public void setAimOffline() {
        fAim = Aim.TO_OFFLINE;
        fLog.add(this.getItemID(), new DALine("Befehl: Alle Module aus..."));
    }

    public void setAimUndock() {
        fAim = Aim.UNDOCK;
        fLog.add(this.getItemID(), new DALine("Befehl: Abdocksequenz einleiten..."));
    }
    ...
    public void alignTo(DAVector3d<Dimensionless> dir) {
        ...
        fLog.add(this.getItemID(), new DALine("Befehl: Ausrichten nach " + dir + "..."));
    }

    public void alignTo(DALocation loc) {
        fLog.add(this.getItemID(), new DALine("Befehl: Ausrichten auf " + loc + "..."));
    ...
    public void alignToDestination() {
        ...
        fAim = Aim.ALIGN_TO;
        fLog.add(this.getItemID(), new DALine("Befehl: Ausrichten auf Ziel..."));
    }
    ...
    public void flyToDestination(DALocation loc) {
        setDestination(loc);
        fLog.add(loc.getWorldNodeID(), new DALine("Befehl: Ziel " + loc + " anfliegen..."));
        flyToDestination();
    }

    public void flyToDestination() {
        fAim = Aim.FLY_TO;
        fLog.add(this.getItemID(), new DALine("Befehl: Ziel anfliegen..."));
    }
...
//        fLog.add(destID, new DALine("Schiff plaziert bei " + dest + "."));
//    }
    ...
    public void stopRoll() {
        ...
        fAim = Aim.STOP_ROLL;
        fLog.add(this.getItemID(), new DALine("Befehl: Stoppe Rollbewegung..."));
    }
    ...
    public void stopStopStrafe() {
        ...
        fLog.add(this.getItemID(), new DALine("Befehl: Stoppe Translation..."));
    }

    public void fullStop() {
        fAim = Aim.FULL_STOP;
        fLog.add(this.getItemID(), new DALine("Befehl: Kompletter Stopp..."));
    }

    @Override
    public synchronized void longTick(DADateTime actWorldTime, DAValue<Duration> t) {
        super.longTick(actWorldTime, t);
        fLog.add(this.getItemID(), new DALine("LongTick."));
        if (market != null) {
            synchronized(market) {
        ...
        switch (fAim) {
            case NONE: {  // Manual flight
                if (isWarping()) {
                    // todo
                    // stop warp engine
                    fLog.add(this.getItemID(), new DALine("Befehl: KEINER. Todo: Stoppe warp."));
                }
                break;
            }
            case TO_IDLE: {
...
//                    aim = Aim.NONE;
                    ...
                    fLog.add(this.getItemID(), new DALine("Alle Maschinen sind betriebsbereit."));
                }
                break;
            }
            case TO_OFFLINE: {
...
//                    aim = Aim.NONE;
                    ...
                    fLog.add(this.getItemID(), new DALine("Alle Maschinen sind aus."));
                }
                break;
            }
            case UNDOCK: {
...
////                    fLog.add(this.getItemID(), new DALine("TODO: Ausrichten auf Abdockposition."));
////                    alignTo();
////                } else {
...
//                        fLog.add(this.getItemID(), new DALine("Abdockposition erreicht."));
//                        break;
//                    }
...
////                }
        ...
        if (fState != State.MANUAL_FLIGHT) {
            ...
            if (! r.isOK()) {
                fLog.add(this.getItemID(), new DALine(toString() + " FEHLER in RCS: " + r.toString()));
...
//    }
    ...
    public synchronized void addLog(DAUniqueID sourceID, DALine message) {
...
//            case SET_NAME: {
//                try {
//                    DALine n = (DALine) parms.get(Parameters.NAME);
//                    if (n == null) {
...
//            case SET_THRUST: {
//                try {
//                    ThrusterType tt = ThrusterType.valueOf(((DALine) parms.get(Parameters.THRUSTER_TYPE)).toString());
//                    Speed ts = Speed.valueOf(((DALine) parms.get(Parameters.THRUSTER_TYPE)).toString());
...
//            case INC_THRUST: {
//                try {
//                    PresetType pt = PresetType.valueOf(((DALine) parms.get(Parameters.PRESET_TYPE)).toString());
...
//            fLog.add(this.getItemID(), new DALine("Fehler bei Zielanflug: Schiff ist nicht bereit. Gehe in Bereitschaft."));
//            Log.debug(DAmcShip.class, "fly_to: " + this + " FlyTo: Ship not ready. Going to idle mode...");
//            setAimIdle();
//            return;
//        }
//        if (destination == null) {
//            fLog.add(this.getItemID(), new DALine("Fehler bei Zielanflug: Kein Ziel gesetzt. Gehe in Bereitschaft."));
...
//            fLog.add(this.getItemID(), new DALine("Zielanflug: Verlasse Orbit..."));
...
//        fLog.add(this.getItemID(), new DALine("Zielanflug: Entfernung " + dist.length() + "m."));
//        if (dist.length() < r) {
//            v.set(new Vector3d(), DVCCoordinateSystem.Type.CARTESIAN_RIGHT_HANDED_XZ);
//            fLog.add(this.getItemID(), new DALine("Zielanflug: Ziel erreicht. Gehe in Bereitschaft."));
...
//            }
//        }
//        fLog.add(this.getItemID(), new DALine(toString() + " Anflug. (" + fState + ")"));
...
//            return DAResult.createOK("Removed captain <" + ch + ">.", "DAmcShip.remove");
    ...
    public DAResult<?> addToCrew(DAPerson p) {
        ...
        if (crew.add(new DALogItem<DAPerson>(p))) {
            fLog.add(p.getID(), new DALine("Crew: " + p + " an Bord."));
    ...
    public DAResult removeFromCrew(DACharacter ch) {
        for (DALogItem<DAPerson> li : crew) {
            if (li.getData() instanceof DACharacter) {
                if (((DACharacter) li.getData()).equals(ch)) {
                    crew.remove(li);
                    fLog.add(ch.getID(), new DALine("Crew: " + ch + " von Bord."));
    ...
    public DAResult removeFromCrew(DAPerson p) {
        for (DALogItem<DAPerson> li : crew) {
            if (li.getData().equals(p)) {
                crew.remove(li);
                fLog.add(p.getID(), new DALine("Crew: " + p + " von Bord."));
                return DAResult.createOK("ok", "DAShip.removeFromCrew");
            }
        }
    ...
    public DAResult addToPassenger(DAPerson p) {
        ...
        if (passengers.add(new DALogItem<DAPerson>(p))) {
            fLog.add(p.getID(), new DALine("Passagier: " + p + " an Bord."));
        ...
        for (DALogItem<DAPerson> li : passengers) {
            if (li.getData() instanceof DACharacter) {
                if (((DACharacter) li.getData()).equals(ch)) {
                    ...
                    fLog.add(ch.getID(), new DALine("Passagier: " + ch + " von Bord."));
    ...
    public DAResult removeFromPassenger(DAPerson p) {
        for (DALogItem<DAPerson> li : passengers) {
            if (li.getData().equals(p)) {
                passengers.remove(li);
                fLog.add(p.getID(), new DALine("Passagier: " + p + " von Bord."));
                return DAResult.createOK("ok", "DAShip.removeFromPassenger");
            }
        }
