...
import static de.hdc.commonlibrary.data.quantity.NewUnits.CREDITS;
...
public class DAOrder extends DataAtom {
    ...
    @Deprecated
    public DAOrder() {
        this(Type.BUY_ORDER, DAValue.create(1, PIECES), DAValue.create(0, CREDITS)
                , new DAWareClass(), null
                , DAUniqueID.createRandom(), DAUniqueID.createRandom(), DAText.create(""));
    }
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        if (in.readBoolean()) {
            if (version == 1) {
                ...
                v.readExternal(in);
                list = new DAWareAmount(v.firstElement().getWare(), v.firstElement().getAmount());
            } else {
                list = new DAWareAmount();
    ...
    private DAOrder(Type aType, DAValue<Pieces> aAmount, DAValue<Money> aPrice, DAWareClass aWareClass
            , DAWareAmount aSellAmount, DAUniqueID aStorageID
    ...
    private DAOrder(Type aType, DAValue<Pieces> aAmount, DAValue<Money> aPrice, DAWareClass aWareClass
            , DADateTime aExpires, DAWareAmount aSellAmount
    ...
    public static DAOrder createSellOrder(DAValue<Money> aPrice, DAWareAmount aSellAmount
            , DAbmStorage aStorage, DAText aDescription) {
        if (aStorage == null) {
    ...
    public static DAOrder createRefillingSellOrder(DAValue<Money> aPrice, DAWareAmount aSellAmount
            , DAbmStorage aStorage, DAText aDescription, DAValue<Pieces> aMaxAmount, DAValue<Duration> aRefillTime) {
        if (aStorage == null) {
...
//    }
    ...
    public DAWareAmount getSellAmount() {
        return list;
    }
    ...
    public DAValue<Pieces> getRefillMaxAmount() {
        return maxAmount;
    }

    public DAValue<Duration> getRefillTime() {
        return refillTime;
    }

    public DAUniqueID getStorageID() {
        return storageID;
    }

    public DAUniqueID getClanID() {
        return clanID;
    }
     ...
     */
    protected DAValue<Pieces> maxAmount;
    /**
     * For refilling orders.
     * Time after which a new order (up to maxAmount) is created.
     */
    protected DAValue<Duration> refillTime;

    /**
     * UTC date when next fillup should happen.
     * Used by DVCMarket.longTick
     */
    protected transient DADateTime nextFillup;

    private DAUniqueID orderID;
    private Type type;
    private DADateTime created;
    private DADateTime expires;
    private DAValue<Money> price;
    /**
     * Amount of ware to buy.
     * Null if SELL_ORDER.
     */
    private DAValue<Pieces> buyAmount;
    /**
     * Type of ware to buy.
     * Null if SELL_ORDER.
     */
    private DAUniqueID wareClassID;
    /**
     * Name of ware to buy.
     * Null if SELL_ORDER.
     */
    private DAText wareClassString;
    /**
     * Ware amounts to sell.
     * Null if BUY_ORDER.
     */
    private DAWareAmount list;
    /** Clan which created this order. */
    private DAUniqueID clanID;
    /** Storage where the ware is (sell) / will go (buy) */
    private DAUniqueID storageID;
